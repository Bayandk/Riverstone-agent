...
from property_functions import FUNCTION_MAP
...
async def sts_receiver(sts_ws, twilio_ws, streamsid_queue):
    ...
    async for message in sts_ws:
        ...
        # Binary μ-law (8k) audio from Deepgram Agent
        raw_mulaw = message

        # DEBUG: confirm we push audio back to Twilio
        try:
            print(f"Sending {len(raw_mulaw)} bytes of audio to Twilio")
        except Exception:
            pass
...
async def twilio_receiver(twilio_ws, audio_queue, streamsid_queue):
    # 20 ms frames at 8kHz μ-law (160 bytes) for lower latency
    BUFFER_SIZE = 160
